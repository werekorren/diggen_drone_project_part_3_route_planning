...
You will write your own code based on {\color{blue}\href{https://github.com/rogerhenriksson/InfoCom-Drone-3-Route_Planning}{this GitHub repository (https://github.com/rogerhenriksson/InfoCom-Drone-3-Route\_Planning)}}. This is very similar to what you were working on in the two previous parts, Drone simulation on Raspberry Pi and Microservices, but you no longer need your joystick or keyboard on Raspberry Pi to control your drone. Instead, you write your own code so that your drone
\begin{itemize}
    \item autonomously plans a path to move along,
    \item sends updates of its real-time location to the Redis database along the way in order to display its position on the map.
\end{itemize}
...
The form asks you to write two addresses, [From address] and [To address]. These are two addresses of your path that you want your drone to fly along. Imagine that the first address is a place you want the drone to go to in order to pick up a parcel, which should then be delivered to the second address. The drone should thus first fly to the first address and then continue on to the second one. You can input any real addresses in Lund, and search for them. The two addresses will be submitted to the server \verb!route_planner.py!, which retrieves their OSM coordinates in (longitude, latitude).
...
\noindent The new route planner server takes the input addresses, and sends it to the OpenStreetMap service using \verb!geopy.geocoders.Nominatim!, which is a Python API that communicates with OpenMapStreet to get the geographical location of a real address. In this application we are only interested in the longitude and latitude values of an address. When you submit the two addresses, you will see a window pop up, telling you whether the server can find the two addresses or not. If it could not, then you need to write a new address.

The route planner will in turn invoke the \verb!/pi/pi_controller.py! script when it finds the two input addresses correctly.
Thus in this project part, you do not need to run this script separately in another terminal -- it is done automatically. Contrary to what you did in the previous project parts, the \verb!/pi/pi_controller.py! in this lab should send its current location in (longitude, latitude) to the database server (instead of the direction of movement). Therefore, in \verb!database.py!, the script only takes the values sent from \verb!/pi/pi_controller.py!, and stores them in Redis, without any further processing.
...
Begin by setting the current target to the from position. In \texttt{your\_function()}, check if you are very close to the target (equal to or less than one step away). If so, return the position of the target. Else, check if you are more than one step away from the target along the X axis and, if so, move one step along the X axis towards the target. Do the same for the Y axis. When you reach the target, and the target is the from position, change the target to the to position.

If you solve the problem like this the drone will only move in the cardinal (north, east, south, west), and intercardinal (northeast, southeast, southwest, northwest) directions. That is fine, but if you like you can instead calculate and return the position one step away from the current position directly in the direction of the target using your geometry skills from high school.

\subsubsection{Math hint}

Suppose the drone is currently located at $(x,y)$ and the target position is $(x_t,y_t)$. The number of remaining steps, assuming each step is $1/10000$ of a degree, is then (the symbols $\lceil$ $\rceil$ means you should round the number to the next bigger integer -- see the \texttt{ceil()} Python function):\\

\begin{math}
n = \lceil \sqrt{(x_t-x)^2+(y_t-y)^2} \cdot 10000 \rceil
\end{math}
\\

\noindent The next drone position can then be calculated like this:\\
...
This method makes use of the observation that the task of moving from one point to another point via a third point can be divided into two easier tasks. The entire contents of the \texttt{run()} method could then be replaced by two calls to a function performing a single trip between two positions -- first from the current position to the from position and then from the from position to the to position:
...
If you choose this approach you no longer need to implement the \texttt{your\_function()} function calculating the next step. It can be fully replaced by the \texttt{travel()} function which handles the entire trip from the start point to the target.

\newpage

\subsubsection{Math hint}

If we want to move from position $(x_1,y_1)$ to position $(x_2,y_2)$ in steps of $1/10000$ of a degree, a suitable number of steps, $n$, is (remember to round it to an integer):\\

\begin{math}
n = \sqrt{(x_2-x_1)^2+(y_2-y_1)^2} \cdot 10000
\end{math}
\\

\noindent The drone position in step number $i$, is then given by:\\

\begin{math}
x_i = x_1 + \frac{i \cdot (x_2 - x_1)}{n}
\end{math}

\begin{math}
y_i = y_1 + \frac{i \cdot (y_2 - y_1)}{n}
\end{math}
